...
function inlineRegex(trigger) {
    const escaped = escapeRegExp(trigger);
    return new RegExp(`(${escaped}).+?(${escaped})$`);
}

function blockRegex(trigger) {
    const escaped = escapeRegExp(trigger);
    return new RegExp(`^(${escaped})$`);
}

function createSetBlockApply(type) {
    return (editor, range) => {
        Transforms.setNodes(
            editor,
            { type },
...
function createSetInlineApply(type) {
    return (editor, range) => {
...
function createSetMarkApply(key) {
    return (editor, range) => {
        Transforms.insertNodes(
            editor,
            { text: " " },
...
function before(editor, at, stringOffset) {
...
export function withMarkdown(editor) {
